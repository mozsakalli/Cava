...
public class SootDecompiler {
    ...
    public static Clazz decompile(String name) {
        ...
        if(c.isInterface && !c.interfaces.isEmpty()) {
            ...
            /*
            Clazz parent = decompile(c.interfaces.remove(0));
            for(Method m : parent.methods) {
                Method nm = new Method();
                nm.declaringClass = c.name;
                nm.name = m.name;
                nm.annotations = m.annotations;
                nm.args = m.args;
                nm.signature = m.signature;
                nm.type = m.type;
                nm.modifiers = m.modifiers;
                //c.methods.add(nm);
                
                System.out.println(nm+" from "+m);
