...
public class VariableSlicer {
    ...
    public VariableSlicer(SootMethod method) {
        this.method = method;
        build(method);
    }
    ...
    private void build(SootMethod method) {
        ...
        buildSlices(body, variablesOfUnit, unitToSlice, slices);
    }
    ...
    static void buildSlices(Body body, Map<Unit, Map<Integer, LocalVariable>> variablesOfUnit,
        ...
        for (Unit u : body.getUnits()) {
            ...
            int idx = getSliceForUnit(visibleLocals, visibleVariables, slicesHash, slices);
            if (idx >= 0) {
    ...
    public UnitVariableSlice getVariableSliceForUnit(Unit unit) {
        return slices.get(getVariableSliceIndexForUnit(unit));
    }

    public UnitVariableSlice getVariableSliceForUnitIfExists(Unit unit) {
        int index = getVariableSliceIndexForUnit(unit);
        return index != -1 ? slices.get(index) : null;
    }
