...
public class IOSApplicationConfiguration {
    ...
    /**
     * Scale factor to use on large screens with retina display, i.e. iPad 3+
     * (has no effect on non-retina screens).
     * <ul>
     * <li>1.0 = no scaling (everything is in pixels)
     * <li>0.5 = LibGDX will behave as you would only have half the pixels. I.e.
     * instead of 2048x1536 you will work in 1024x768. This looks pixel perfect
     * and will save you the trouble to create bigger graphics for the retina
     * display.
     * <li>any other value: scales the screens according to your scale factor. A
     * scale factor oof 0.75, 0.8, 1.2, 1.5 etc. works very well without any
     * artifacts!
     * </ul>
     */
    public float displayScaleLargeScreenIfRetina = 1.0f;
    /**
     * Scale factor to use on small screens with retina display, i.e. iPhone 4+,
     * iPod 4+ (has no effect on non-retina screens).
     * <ul>
     * <li>1.0 = no scaling (everything is in pixels)
     * <li>0.5 = LibGDX will behave as you would only have half the pixels. I.e.
     * instead of 960x640 you will work in 480x320. This looks pixel perfect and
     * will save you the trouble to create bigger graphics for the retina
     * display.
     * <li>any other value: scales the screens according to your scale factor. A
     * scale factor of 0.75, 0.8, 1.2, 1.5 etc. works very well without any
     * artifacts!
     * </ul>
     */
    public float displayScaleSmallScreenIfRetina = 1.0f;
    /**
     * Scale factor to use on large screens without retina display, i.e. iPad
     * 1+2 (has no effect on retina screens).
     * <ul>
     * <li>1.0 = no scaling (everything is in pixels)
     * <li>any other value: scales the screens according to your scale factor. A
     * scale factor of 0.75, 0.8, 1.2, 1.5 etc. works very well without any
    ...
    /**
     * Scale factor to use on small screens without retina display, i.e. iPhone
     * 1-3, iPod 1-3 (has no effect on retina screens).
     * <ul>
     * <li>1.0 = no scaling (everything is in pixels)
     * <li>any other value: scales the screens according to your scale factor. A
     * scale factor of 0.75, 0.8, 1.2, 1.5 etc. works very well without any
    ...
    /**
     * Experimental, whether to enable OpenGL ES 3 if supported. If not
     * supported it will fall-back to OpenGL ES 2.0. When GL ES 3 is enabled,
     * {@link com.badlogic.gdx.Gdx#gl30} can be used to access it's
     * functionality.
     *
     * @deprecated this option is currently experimental and not yet fully
     * supported, expect issues.
     */
    @Deprecated
    ...
    /**
     * Whether to override the ringer/mute switch, see
     * https://github.com/libgdx/libgdx/issues/4430
     */
    public boolean overrideRingerSwitch = false;

    /**
     * Edges where app gestures must be fired over system gestures. Prior to iOS
     * 11, UIRectEdge.All was default behaviour if status bar hidden, see
     * https://github.com/libgdx/libgdx/issues/5110 *
     */
    //todo: public UIRectEdge screenEdgesDeferringSystemGestures = UIRectEdge.None;

    /**
     * The maximum number of threads to use for network requests. Default is
    ...
    /**
     * adds device information for newer iOS devices, or overrides information
     * for given ones
     *
