...
gdx2d_pixmap* gdx2d_load(const unsigned char *buffer, uint32_t len) {
	...
	if (pixels == NULL) {
        //todo: pixels = jpgd_decompress_jpeg_image_from_memory(buffer, len, &width, &height, &format, 3);
	}
	if (pixels == NULL)
		return NULL;
...
const char *gdx2d_get_failure_reason(void) {
    ...
    return NULL; //todo: jpgd_failure_reason();
}
...
jobject mcom_badlogic_gdx_graphics_g2d_Gdx2DPixmap_load___J_BII_Ljava_nio_ByteBuffer_(jobject nativeData, jobject buffer, jint offset, jint len) {
    ...
    java_nio_ByteBuffer* pixel_buffer = (java_nio_ByteBuffer*)JvmAllocObject(&java_nio_ByteBuffer_Class);
    pixel_buffer->fjava_nio_Buffer_capacity = pixel_buffer->fjava_nio_Buffer_limit =
